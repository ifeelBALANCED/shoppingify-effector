 ...
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */var printWarning$1=function printWarning$1(){};if(process.env.NODE_ENV!=='production'){var ReactPropTypesSecret$2=ReactPropTypesSecret_1;var loggedTypeFailures={};var has$5=has$6;printWarning$1=function printWarning$1(text){var message='Warning: '+text;if(typeof console!=='undefined'){console.error(message);}try{// --- Welcome to debugging React ---
 ...
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */var ReactIs$1=reactIs$1.exports;var assign$1=objectAssign;var ReactPropTypesSecret$1=ReactPropTypesSecret_1;var has$4=has$6;var checkPropTypes=checkPropTypes_1;var printWarning=function printWarning(){};if(process.env.NODE_ENV!=='production'){printWarning=function printWarning(text){var message='Warning: '+text;if(typeof console!=='undefined'){console.error(message);}try{// --- Welcome to debugging React ---
// This error was thrown as a convenience so that you can use this stack
// to find the callsite that caused this warning to fire.
 ...
 * _.isSymbol('abc');
 * // => false
 */function isSymbol$2(value){return _typeof2(value)=='symbol'||isObjectLike(value)&&baseGetTag(value)==symbolTag;}var isSymbol_1=isSymbol$2;var baseTrim=_baseTrim,isObject$2=isObject_1,isSymbol$1=isSymbol_1;/** Used as references for various `Number` constants. */var NAN=0/0;/** Used to detect bad signed hexadecimal string values. */var reIsBadHex=/^[-+]0x[0-9a-f]+$/i;/** Used to detect binary string values. */var reIsBinary=/^0b[01]+$/i;/** Used to detect octal string values. */var reIsOctal=/^0o[0-7]+$/i;/** Built-in method references without a dependency on `root`. */var freeParseInt=parseInt;/**
...
function isEqual(val1,val2){return val1===val2||val1!==val1&&val2!==val2;}var get$1=function get$1(){try{var data=store.local.get("storybook-layout");return data||false;}catch(e){// eslint-disable-next-line no-console
console.error(e);return false;}};var write=memoizerific(1)(function(changes){try{store.local.set("storybook-layout",changes);}catch(e){// eslint-disable-next-line no-console
console.error(e);}});var set=debounce_1(write,500);var cjs={exports:{}};var Draggable$2={};function toVal(mix){var k,y,str='';if(typeof mix==='string'||typeof mix==='number'){str+=mix;}else if(_typeof2(mix)==='object'){if(Array.isArray(mix)){for(k=0;k<mix.length;k++){if(mix[k]){if(y=toVal(mix[k])){str&&(str+=' ');str+=y;}}}}else{for(k in mix){if(mix[k]){str&&(str+=' ');str+=k;}}}}return str;}function clsx_m(){var i=0,tmp,x,str='';while(i<arguments.length){if(tmp=arguments[i++]){if(x=toVal(tmp)){str&&(str+=' ');str+=x;}}}return str;}var clsx_m$1=/*#__PURE__*/Object.freeze({__proto__:null,'default':clsx_m});var require$$3=/*@__PURE__*/getAugmentedNamespace(clsx_m$1);var domFns={};var shims={};Object.defineProperty(shims,"__esModule",{value:true});shims.findInArray=findInArray;shims.isFunction=isFunction;shims.isNum=isNum;shims.int=int;shims.dontSetMe=dontSetMe;// @credits https://gist.github.com/rogozhnikoff/a43cfed27c41e4e68cdc
...
useEffect(function(){var menuElement=document$5.getElementById('storybook-explorer-menu');// Even though we ignore repeated events, use throttle because IE doesn't support event.repeat.
...
var singleStoryComponentIds=useMemo(function(){return orphansFirst.filter(function(nodeId){var _data$nodeId=data[nodeId],_data$nodeId$children=_data$nodeId.children,children=_data$nodeId$children===void 0?[]:_data$nodeId$children,isComponent=_data$nodeId.isComponent,isLeaf=_data$nodeId.isLeaf,name=_data$nodeId.name;return!isLeaf&&isComponent&&children.length===1&&isStory(data[children[0]])&&isStoryHoistable(data[children[0]].name,name);});},[data,orphansFirst]);// Omit single-story components from the list of nodes.
...
useEffect(function(){var menuElement=document$4.getElementById('storybook-explorer-menu');var lastRequestId;var navigateTree=function navigateTree(event){if(isLoading||!isBrowsing||!containerRef.current)return;// allow event.repeat
